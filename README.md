...
# 命令行调试但不真的锁屏
python main.py --cli --no-lock

# 完整诊断：检查配置 + 空闲检测 + 连续 BLE 扫描
...
1. 正常使用电脑时，不持续扫描蓝牙。
2. 本地空闲达到 `idle_grace_seconds` 后，进入 BLE 检测。
3. 系统会先确认过手机确实在附近，之后才会用短时弱信号/丢信号判断你已经离开。
4. 如果一直没有确认到手机在附近，也必须持续较长时间都弱信号/无信号，才会锁屏。
5. 检测到本地重新有操作后，只做静默复位，不主动亮屏。
